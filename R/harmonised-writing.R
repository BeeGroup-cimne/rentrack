...
ENTRACK_namespaces <- c("entrack" = "http://en-track.eu/ontology#",
...
#' @param namespaces named <array>. Example: 'rentrack::ENTRACK_namespaces'.
...
#
# Specific functions to harmonise resultant datasets to EN-TRACK Ontology ----
#

#' Generate the harmonised longitudinal benchmarking results
#' 
#' Generate the results in harmonised format to EN-TRACK ontology for the energy consumption benchmarking of 
...
#' (E.g.: http://EN-TRACK-project.eu/ontology#EnergyConsumptionGridElectricity)
#' @param measuredPropertyComponent <uri> defining the energy consumption component name. 
#' (E.g.: "http://EN-TRACK-project.eu/ontology#Heating").
...
generate_longitudinal_benchmarking_indicators <- function (
  ...
  namespaces["rentrackesults"] <- buildingNamespace
  ...
  if (!is.null(modelId)) {
    modelSubject <- sprintf("rentrackesults:MODEL-%s-%s", buildingId, modelId)
    if (!exists_analytical_model(obj, modelSubject, namespaces)) {
      obj <- obj %>% add_item_to_rdf(subject = modelSubject, 
                                     classes = c("entrack:AnalyticalModel"), dataProperties = c(
                                       list(`entrack:modelLocation` = modelLocation, 
                                            `entrack:modelTrainedDate` = format_iso_8601z(lubridate::now("UTC")), 
                                            `entrack:modelName` = modelName),
                                       if (!is.null(modelBaselineYear)) {
                                         list(`entrack:modelBaselineYear` = modelBaselineYear)
                                       }), 
                                     objectProperties = list(
                                       `entrack:hasModelStorageInfrastructure` = modelStorageInfrastructureSubject, 
                                       `entrack:hasModelType` = modelTypeSubject), namespaces = namespaces)
      obj <- obj %>% add_item_to_rdf(subject = buildingSubject, 
                                     objectProperties = list(`entrack:hasAnalyticalModel` = modelSubject), 
  ...
  for (indicator in indicators) {
    ...
    for (frequency in frequencies_) {
      ...
      keyPerformanceIndicatorSubject <- paste("entrack:KPI", indicator, sep = "-")
      singleKPISubject <- paste("rentrackesults:SingleKPI", buildingId, 
      ...
      singleKPIPointSubject <- paste0("rentrackesults:", singleKPISubjectHash)
      obj %>% add_item_to_rdf(subject = singleKPISubject, 
                              classes = c("entrack:SingleKPIAssessment", "entrack:KPIAssessment", 
                                          "entrack:TimeSeriesList"), 
                              dataProperties = c(list(
                                `entrack:timeSeriesIsRegular` = T, 
                                ...
                                `entrack:timeSeriesTimeAggregationFunction` = "SUM")), 
                              objectProperties = c(list(
                                `entrack:hasKPIUnit` = indicatorsUnitsSubjects[[indicator]], 
                                `entrack:hasSingleKPIPoint` = singleKPIPointSubject, 
                                `entrack:quantifiesKPI` = keyPerformanceIndicatorSubject),
                                if (!is.null(modelId)) { list(
                                  `entrack:isEstimatedByModel` = modelSubject)
                                },
                                if (!(indicator %in% c("HeatingDegreeDays", "CoolingDegreeDays"))) { list(
                                  `entrack:hasMeasuredProperty` = if(startsWith(measuredProperty,"entrack:")){ measuredProperty
                                  } else {paste0("entrack:",measuredProperty)}) },
                                if (!(indicator %in% c("HeatingDegreeDays", "CoolingDegreeDays"))) { list(
                                  `entrack:hasMeasuredPropertyComponent` = paste0("entrack:",measuredPropertyComponent)) }
                              ), namespaces = namespaces)
      obj %>% add_item_to_rdf(subject = singleKPIPointSubject,
                              classes = c("entrack:TimeSeriesPoint","entrack:SingleKPIAssessmentPoint"),
                              namespaces=namespaces)
      obj %>% add_item_to_rdf(subject = buildingSubject, 
                              objectProperties = list(`entrack:assessesSingleKPI` = singleKPISubject), 
                              namespaces = namespaces)
      ...
      if(lubridate::as.period(frequency)>=lubridate::as.period("P1M")){
        indDfAuxMeta <- data.frame(
          ...
          `measuredProperty` = namespace_integrator(paste0("entrack:",measuredProperty), namespaces),
          `measuredPropertyComponent` = namespace_integrator(paste0("entrack:",measuredPropertyComponent), namespaces),
          ...
          `modelName` = if(is.na(modelSubject)){NA}else{namespace_integrator(paste0("entrack:",modelName), namespaces)},
...
#' 
#' Generate the results in harmonised format to EN-TRACK ontology for the EEM assessment in buildings. 
...
generate_eem_assessment_indicators <- function(
  ...
  if("eemProjectSubject" %in% colnames(eemProjectDf)){
    ...
    projectSubject <- sprintf("rentrackesults:PROJECT-%s-%s",buildingId,projectId)
    obj <- obj %>% add_item_to_rdf(subject = buildingSubject, 
                                   objectProperties = list(`entrack:hasProject` = projectSubject), 
                                   namespaces = namespaces)
    if (!exists_project_model(obj, projectSubject, namespaces)) {
      obj <- obj %>% add_item_to_rdf(subject = projectSubject, 
                                     classes = c("entrack:Project"),
                                     dataProperties = c(list(
                                       `entrack:projectName` = sprintf("Project %s",projectId),
                                       `entrack:projectDescription` = "Automatically created by the EEM assessment pipeline",
                                       `entrack:projectAutoGenerated` = T,
                                       `entrack:projectGenerationDate` = format_iso_8601z(lubridate::now("UTC")),
                                       `entrack:projectOperationalDate` = format_iso_8601z(lubridate::with_tz(max(eemProjectDf$Date),"UTC")),
                                       `entrack:projectStartDate` = format_iso_8601z(lubridate::with_tz(min(eemProjectDf$Date),"UTC")),
                                       `entrack:projectEndDate` = format_iso_8601z(lubridate::with_tz(max(eemProjectDf$Date),"UTC")),
                                       `entrack:projectNumberEEMs` = nrow(eemProjectDf)),
                                       if(length(unique(eemProjectDf$Currency))==1){
                                         list(`entrack:projectInvestment` = sum(eemProjectDf$Investment,na.rm=T))
                                       }),
                                     objectProperties = c(
                                       if(length(unique(eemProjectDf$Currency))==1){
                                         list(`entrack:hasProjectInvestmentCurrency` = eemProjectDf$Currency[1])
                                       },
                                       # setNames(as.list(eemProjectDf$Type[order(eemProjectDf$Investment,decreasing = T)]),
                                       #          rep("entrack:hasEnergyEfficiencyMeasureType",nrow(eemProjectDf))),
                                       # list(`entrack:affectsBuilding` = buildingSubject),
                                       setNames(as.list(eemProjectDf$eemSubject),rep("entrack:includesMeasure",nrow(eemProjectDf)))), 
                                     namespaces = namespaces)
    }
  }
  ...
  if (!is.null(modelId)) {
    ...
    if (!exists_analytical_model(obj, modelSubject, namespaces)) {
      obj <- obj %>% add_item_to_rdf(subject = modelSubject, 
                                     classes = c("entrack:AnalyticalModel"), dataProperties =
                                       list(`entrack:modelLocation` = modelLocation, 
                                            `entrack:modelTrainedDate` = format_iso_8601z(lubridate::now("UTC")), 
                                            `entrack:modelName` = modelName), 
                                     objectProperties = list(
                                       `entrack:hasModelStorageInfrastructure` = modelStorageInfrastructureSubject, 
                                       `entrack:hasModelType` = "entrack:BaselineModel"), namespaces = namespaces)
      obj <- obj %>% add_item_to_rdf(subject = buildingSubject, 
                                     objectProperties = list(`entrack:hasAnalyticalModel` = modelSubject), 
                                     namespaces = namespaces)
    }
  ...
  for (indicator in indicatorsAggregatableByTime) {
    ...
    for (frequency in frequencies) {
      ...
      keyPerformanceIndicatorSubject <- paste("entrack:KPI", indicator, sep = "-")
      singleKPISubject <- paste("rentrackesults:SingleKPI", buildingId, projectId,
      ...
      obj %>% add_item_to_rdf(subject = singleKPISubject, 
                              classes = c("entrack:SingleKPIAssessment", "entrack:KPIAssessment", 
                                          "entrack:TimeSeriesList"), 
                              dataProperties = c(list(
                                `entrack:timeSeriesIsRegular` = if(frequency==""){F}else{T}, 
                                `entrack:timeSeriesIsOnChange` = F, 
                                `entrack:timeSeriesIsCumulative` = F, 
                                `entrack:timeSeriesStart` = min(indDfAux$start, na.rm = T), 
                                `entrack:timeSeriesEnd` = max(indDfAux$end,  na.rm = T),
                                `entrack:timeSeriesTimeAggregationFunction` = indicatorsTimeAggregationFunctions[[indicator]]),
                              if(frequency!=""){list(
                                `entrack:timeSeriesFrequency` = frequency
                              )}), 
                              objectProperties = c(list(
                                ...
                                `entrack:hasSingleKPIPoint` = singleKPIPointSubject, 
                                `entrack:quantifiesKPI` = keyPerformanceIndicatorSubject,
                                `entrack:hasMeasuredProperty` = if(startsWith(measuredProperty,"entrack:")){ 
                                  measuredProperty
                                } else {paste0("entrack:",measuredProperty)},
                                `entrack:hasMeasuredPropertyComponent` = if(startsWith(measuredPropertyComponent,"entrack:")){ 
                                  measuredPropertyComponent
                                } else {paste0("entrack:",measuredPropertyComponent)}),
                                if (!is.null(modelId)) { list(
                                  `entrack:isEstimatedByModel` = modelSubject)
                                }
                              ), namespaces = namespaces)
      ...
      obj %>% add_item_to_rdf(subject = projectSubject, 
                              classes = "entrack:KPICalculationItem",
                              objectProperties = list(`entrack:assessesSingleKPI` = singleKPISubject), 
                              namespaces = namespaces)
      indDfAux$start <- format_iso_8601z(indDfAux$start)
      indDfAux$end <- format_iso_8601z(indDfAux$end)
  ...
  if(any(is.na(frequencies) | frequencies=="") && is.finite(annualEnergySavings) && 
    ...
    for (indicatorNABT in indicatorsNonAggregatableByTime){
      ...
      keyPerformanceIndicatorSubject <- paste("entrack:KPI", indicatorNABT, sep = "-")
      singleKPISubject <- paste("rentrackesults:SingleKPI", buildingId, projectId,
      ...
      obj %>% add_item_to_rdf(subject = singleKPISubject, 
                              classes = c("entrack:SingleKPIAssessment", "entrack:KPIAssessment", 
                                          "entrack:TimeSeriesList"), 
                              dataProperties = list(
                                `entrack:timeSeriesIsRegular` = F, 
                                `entrack:timeSeriesIsOnChange` = F, 
                                `entrack:timeSeriesIsCumulative` = F, 
                                `entrack:timeSeriesStart` = min(indDfAux$start, na.rm = T), 
                                `entrack:timeSeriesEnd` = max(indDfAux$end,  na.rm = T)),
                              objectProperties = c(list(
                                `entrack:hasKPIUnit` = indicatorsUnitsSubjects[[indicatorNABT]], 
                                `entrack:hasSingleKPIPoint` = singleKPIPointSubject, 
                                `entrack:quantifiesKPI` = keyPerformanceIndicatorSubject,
                                `entrack:hasMeasuredPropertyComponent` = "entrack:Total",
                                `entrack:hasMeasuredProperty` = if(startsWith(measuredProperty,"entrack:")){ 
                                  measuredProperty
                                } else {paste0("entrack:",measuredProperty)}),
                                if (!is.null(modelId)) { list(
      ...
      if(frequency=="" | lubridate::as.period(frequency)>=lubridate::as.period("P1M")){
        indDfAuxMeta <- data.frame(
          ...
          `measuredProperty` = namespace_integrator(paste0("entrack:",measuredProperty), namespaces),
          `measuredPropertyComponent` = namespace_integrator("entrack:Total", namespaces),
          `unit` = namespace_integrator(indicatorsUnitsSubjects[[indicatorNABT]], namespaces),
          `frequency` = frequency,
          `modelSubject` = namespace_integrator(modelSubject, namespaces),
          `modelName` = namespace_integrator(paste0("entrack:",modelName), namespaces),
          `modelBased` = !is.na(modelSubject)
        )
        results_ts[[singleKPISubjectHash]]$full <- cbind(indDfAux,indDfAuxMeta)
        results_ts[[singleKPISubjectHash]]$full <- 
    ...
    for (eemElem in 1:nrow(eemProjectDf)){
      ...
      for (indicator in indicatorsAggregatableByTime) {
        ...
        for (frequency in frequencies) {
          ...
          keyPerformanceIndicatorSubject <- paste("entrack:KPI", indicator, sep = "-")
          singleKPISubject <- paste("rentrackesults:SingleKPI", uri_to_identifier(eemSingleDf$eemSubject),
                                    paste(indicator, measuredPropertyComponent, measuredProperty, sep = "-"), 
                                    modelName, modelId, frequency, sep = "-")
          singleKPISubjectHash <- digest::digest(namespace_integrator(singleKPISubject, namespaces), "sha256", serialize = T)
          singleKPIPointSubject <- paste0("rentrackesults:", singleKPISubjectHash)

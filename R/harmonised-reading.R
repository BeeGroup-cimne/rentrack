...
#' 
#' This function get from a EN-TRACK-harmonised dataset the timezone of a list of buildings.
#'
...
get_tz_building <- function(buildingsRdf, buildingSubjects){
  metadata_df <- suppressMessages(buildingsRdf %>% rdf_query(paste0(    
    paste0(mapply(function(i){
      sprintf('PREFIX %s: <%s>', names(ENTRACK_namespaces)[i],
              ENTRACK_namespaces[i])},
      1:length(ENTRACK_namespaces))),
    '
    SELECT ?b ?tz
    WHERE {
      ?b a entrack:Building .
      FILTER ( ?b IN (<',paste(buildingSubjects,collapse='>,<'),'>) ) .
      ?b entrack:hasLocationInfo ?l .
      ?l entrack:addressTimeZone ?tz .
    }')))
...
#' 
#' This function get from a EN-TRACK-harmonised dataset the gross floor area of a list of buildings.
#'
#' @param buildingsRdf <rdf> containing the information of a set of buildings.
#' @param buildingSubjects <array> of URIs of the building subjects.
...
get_area_building <- function(buildingsRdf, buildingSubjects){
  metadata_df <- suppressMessages(buildingsRdf %>% rdf_query(paste0(    
    ...
    WHERE {
      ?b a entrack:Building .
      FILTER ( ?b IN (<',paste(buildingSubjects,collapse='>,<'),'>) ) .
      ?b entrack:hasSpace ?s .
      ?s entrack:hasArea ?a .
      ?a entrack:hasAreaType ?types .
      FILTER regex(str(?types),"GrossFloorArea$") .
      ?a entrack:areaValue ?area .
    }')))
...
#' 
#' This function get from a EN-TRACK-harmonised dataset the namespaces of a list of buildings.
#'
#' @param buildingsRdf <rdf> containing the information of a set of buildings.
#' @param buildingSubjects <array> of URIs of the building subjects.
...
get_buildings_subjects <- function(buildingsRdf, buildingIdsFromOrganization){
  metadata_df <- suppressMessages(buildingsRdf %>% rdf_query(paste0(    
    ...
    WHERE {
      ?b a entrack:Building .
      ?b entrack:buildingIDFromOrganization ?o .
...
#' 
#' This function get all building subjects available from a EN-TRACK-harmonised dataset.
#'
#' @param buildingsRdf <rdf> containing the information of a set of buildings.
#' @param buildingSubjects <array> of URIs of the building subjects.
#' @return <array> with the subject URI for each building.
...
#' 
#' This function checks if certain analytical model subject exists in a EN-TRACK-harmonised dataset.
#'
#' @param buildingsRdf <rdf> containing the information of a set of buildings.
    ...
    WHERE {
      {
        ...
        WHERE {
          ?m a entrack:Sensor.
          ?m entrack:hasMeasurement ?hasMeasurement .
          FILTER regex(str(?hasMeasurement), "',sensorId,'")
        }
      }
      optional {?m entrack:timeSeriesTimeAggregationFunction ?timeSeriesTimeAggregationFunction .}
      optional {?m entrack:timeSeriesIsCumulative ?timeSeriesIsCumulative .}
      optional {?m entrack:timeSeriesIsOnChange ?timeSeriesIsOnChange .}
      optional {?m entrack:timeSeriesIsRegular ?timeSeriesIsRegular .}
      optional {?m entrack:hasMeasuredProperty ?measuredProperty .}
      optional {?m entrack:timeSeriesFrequency ?timeSeriesFrequency .}
      optional {?m entrack:hasEstimationMethod ?hasEstimationMethod .}
      optional {?hasEstimationMethod entrack:considerEstimatedValues ?considerEstimatedValues .}
    }')))
  metadata_df$tz <- tz
  metadata_df$measuredProperty <- gsub(paste0(ENTRACK_namespaces,collapse="|"),"",
...
#'  
#' This function gets all the device aggregators available in a EN-TRACK-harmonised dataset.
    ...
    WHERE {
      {
        ...
        WHERE {
          ?buildingSubject a entrack:Building .
          ?buildingSubject entrack:hasSpace ?bs .
          ?bs entrack:hasDeviceAggregator ?s .
        }
      }
      optional {?s entrack:deviceAggregatorName ?deviceAggregatorName .}
      optional {?s entrack:deviceAggregatorFrequency ?deviceAggregatorFrequency .}
      optional {?s entrack:deviceAggregatorTimeAggregationFunction ?deviceAggregatorTimeAggregationFunction .}
      optional {?s entrack:deviceAggregatorFormula ?deviceAggregatorFormula .}
      optional {?s entrack:hasDeviceAggregatorProperty ?measuredProperty .}
    }')))
  result$measuredProperty <- gsub(paste0(ENTRACK_namespaces,collapse="|"),"",
...
#' aggregators available in a EN-TRACK-harmonised dataset. 
#' 
#' @param buildingsRdf <rdf> containing the information of a set of buildings.
#' @param timeseriesObject <string> path of JSON files, or <list> of time series.
...
#' 
#' This function checks if a certain project subject exists in a EN-TRACK-harmonised dataset.
#'
#' @param buildingsRdf <rdf> containing the information of a set of buildings.
...
get_building_eems <- function(buildingsRdf, buildingSubjects=NULL){
  result <- suppressMessages(buildingsRdf %>% rdf_query(paste0(
    ...
    ifelse(!is.null(buildingSubjects),paste0('
           ...
           ''),
    '?buildingSubject entrack:hasSpace ?bs .
          ?bs entrack:isAssociatedWithElement ?buildingElement .
        }
      }
      optional {?buildingElement entrack:isAffectedByMeasure ?eemSubject .}
    }')))
...
#' 
#' This function get the metadata of a set of EEMs in a EN-TRACK-harmonised dataset. 
...
get_eem_details <- function(buildingsRdf, eemSubjects=NULL){
  result <- suppressMessages(buildingsRdf %>% rdf_query(paste0(
    ...
    WHERE {
      ?eemSubject a entrack:EnergyEfficiencyMeasure .',
    ifelse(!is.null(eemSubjects),paste0('
            FILTER ( ?eemSubject IN (<',paste(eemSubjects,collapse='>,<'),'>) ) .'),'
            '),
    'optional { ?eemSubject entrack:energyEfficiencyMeasureCurrencyExchangeRate ?ExchangeRate .}
       optional { ?eemSubject entrack:energyEfficiencyMeasureDescription ?Description .}
       optional { ?eemSubject entrack:energyEfficiencyMeasureInvestment ?Investment .}
       optional { ?eemSubject entrack:energyEfficiencyMeasureOperationalDate ?Date .}
       optional { ?eemSubject entrack:hasEnergyEfficiencyMeasureInvestmentCurrency ?Currency .}
       optional { ?eemSubject entrack:hasEnergyEfficiencyMeasureType ?Type .}
       optional { ?eemSubject entrack:shareOfAffectedElement ?AffectationShare .}
    }')))
...
get_eem_projects <- function(buildingsRdf, buildingSubject, eemSubjects=NULL){
  result <- suppressMessages(buildingsRdf %>% rdf_query(paste0(
    ...
    WHERE {
      ?eemProjectSubject a entrack:Project .
      ?eemProjectSubject entrack:affectsBuilding ?buildingSubject .',
      paste0('FILTER ( ?buildingSubject IN (<',paste(buildingSubject,collapse='>,<'),'>) ) .'),'
      ?eemProjectSubject entrack:includesMeasure ?eemSubject .',
      ifelse(!is.null(eemSubjects),paste0('
            FILTER ( ?eemSubject IN (<',paste(eemSubjects,collapse='>,<'),'>) ) .'),'
            '),
      'optional { ?eemProjectSubject entrack:projectDescription ?Description .}
       optional { ?eemProjectSubject entrack:projectInvestment ?Investment .}
       optional { ?eemProjectSubject entrack:projectOperationalDate ?DateEnd .}
       optional { ?eemProjectSubject entrack:projectStartDate ?DateStart .}
    }')))
  if(nrow(result)>0) {
...
#' It must be harmonised to EN-TRACK Ontology.
#' @param timeseriesObject <array> of strings with paths to JSON files containing time series,
#' or <list> of time series. It must be harmonised to EN-TRACK Ontology.
    ...
    WHERE {
      {
        ...
        WHERE {
          <',buildingSubject,'> entrack:assessesSingleKPI ?SingleKPI .
          FILTER regex(str(?SingleKPI),"',name,'") .
          ?SingleKPI entrack:timeSeriesFrequency "',frequency,'".
          ?SingleKPI entrack:hasSingleKPIPoint ?uriTimeSeries .
        }
      }
      OPTIONAL {?SingleKPI entrack:isEstimatedByModel ?est .}
      FILTER (',if(fromModel){""}else{"!"},'BOUND(?est))
      {
        OPTIONAL {?est entrack:modelTrainedDate ?date .}
      } UNION {
...
get_tariff_metadata <- function(buildingsRdf, sensorId){
  metadata_df <- suppressMessages(buildingsRdf %>% rdf_query(paste0(    
    ...
    SELECT ?tariff
    WHERE {
      ?upd a entrack:UtilityPointOfDelivery.
      ?upd entrack:hasDevice ?dp.
      ?dp entrack:hasSensor ?m.
      ?m entrack:hasMeasurement ?hasMeasurement .
      FILTER regex(str(?hasMeasurement), "',sensorId,'")
      ?upd entrack:hasContractedTariff ?tariff.
    }')))
  metadata_df$sensorId <- sensorId
  return(metadata_df)
}
...
get_emissions_metadata <- function(buildingsRdf, sensorId){
  metadata_df <- suppressMessages(buildingsRdf %>% rdf_query(paste0(    
    ...
    WHERE {
      ...
      ?m entrack:hasMeasurement ?hasMeasurement .
      FILTER regex(str(?hasMeasurement), "',sensorId,'")
      ?upd entrack:hasCO2EmissionsFactor ?emissions.
    }')))
  metadata_df$sensorId <- sensorId
  return(metadata_df)
}
...
#' sensor in a EN-TRACK-harmonised dataset.
#'
#' @param buildingsRdf <rdf> containing all metadata about buildings. 
#' It must be harmonised to EN-TRACK Ontology.
...
append_cost_to_sensor <- function(buildingsRdf, timeseriesObject, tariffSubject, measuredProperty,
  ...
  tariffSubject <- namespace_integrator(tariffSubject,ENTRACK_namespaces)
  metadata_df <- suppressMessages(buildingsRdf %>% rdf_query(paste0(    
    paste0(mapply(function(i){
      sprintf('PREFIX %s: <%s>', names(ENTRACK_namespaces)[i],
              ENTRACK_namespaces[i])},
    ...
    WHERE {
      {
        SELECT ?ct 
        WHERE {
          ?ct a entrack:ContractedTariff .
          FILTER (?ct = <',tariffSubject,'>) .
        }
      }
      ?ct entrack:hasTariff ?tar .
      ?ct entrack:contractStartDate ?start .
      optional { ?ct entrack:contractEndDate ?end . }
      ?tar entrack:hasTariffComponentList ?tcl.
      ?tcl entrack:hasTariffMeasuredProperty ?measuredProperty.
      ?tcl entrack:timeSeriesFrequency ?timeSeriesFrequency.
      ?tcl entrack:hasTariffComponentPoint ?hash.
    }')))
...
append_emissions_to_sensor <- function(buildingsRdf, timeseriesObject, emissionsSubject, 
  ...
  emissionsSubject <- namespace_integrator(emissionsSubject,ENTRACK_namespaces)
  metadata_df <- suppressMessages(buildingsRdf %>% rdf_query(paste0(    
    paste0(mapply(function(i){
      sprintf('PREFIX %s: <%s>', names(ENTRACK_namespaces)[i],
              ENTRACK_namespaces[i])},
    ...
    WHERE {
      {
        SELECT ?em
        WHERE {
          ?em a entrack:CO2EmissionsFactor .
          FILTER (?em = <',emissionsSubject,'>) .
        }
      }
      ?em entrack:hasCO2EmissionsFactorList ?efl.
      ?efl entrack:hasCO2RelatedMeasuredProperty ?measuredProperty.
      ?efl entrack:timeSeriesFrequency ?timeSeriesFrequency.
      ?efl entrack:hasCO2EmissionsFactorValue ?hash.
    }')))
  if(nrow(metadata_df)==0){
    return(energyTimeseriesSensor)
  } else {

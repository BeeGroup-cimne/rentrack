  ...
  analytical_groups_by_building <- 
    suppressMessages(buildingsRdf %>% rdf_query(paste0(    
      paste0(mapply(function(i){
        sprintf('PREFIX %s: <%s>', names(EN-TRACK_namespaces)[i],
                EN-TRACK_namespaces[i])},
        1:length(EN-TRACK_namespaces))),
      '
      SELECT ?buildingSubject ?group
      WHERE {
        ?buildingSubject a EN-TRACK:Building .
        ?buildingSubject EN-TRACK:groupsForAnalytics ?group
      }')))
...
get_matrix_building_space_use_types <- function(buildingsRdf, splitSublevels=T){
  types_df <- suppressMessages(buildingsRdf %>% rdf_query(paste0(    
    ...
    WHERE {
      ?buildingSubject a EN-TRACK:Building .
      ?buildingSubject EN-TRACK:hasSpace ?bs .
      ?bs EN-TRACK:hasBuildingSpaceUseType ?typology
    }')))
